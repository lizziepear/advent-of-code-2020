...
rm(i, ind.empty, tmp, v, v.ans, v.new)
...
Split on empty rows so you get one string per group of answers. BUT this time, concatenate using `;` so that I can split them out again later and get the answers _by each person_ in each group rather than just the answers for each group.
...
* Find how many letters are shared between _all_ the people in the group (by intersecting 1 with 2, intersecting that result with 3, etc.)
